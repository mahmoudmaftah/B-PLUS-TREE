...
// Include your naive vector index header
#include "../../include/naivevectorindex.h" 
using namespace std;
int main() {
    // Open data.csv
    std::ifstream dataFile("../_Data/_data.csv");
    ...
    // Count columns to deduce D
    {
        ...
        std::ifstream queriesFile("../_Data/_queries.csv");
        ...
        // Count columns to deduce query dimension
        {
            ...
            while (std::getline(queriesFile, line)) {
                ...
                // O is not used by the naive solution, but we'll read and ignore it
                if (!std::getline(qs, col, ',')) {std::cerr<<"Error reading O\n";return 1;}
                // int O = std::stoi(col); // Not needed for naive solution

                // Run query
                std::vector<int> neighbors = index.query(qvec, k, Smin, Smax);
